const char java_colors[] PROGMEM = R"=====(
...
<html>
<head>
    <!-- Include iro.js via CDN -->
    <script src="java_colors.js"></script>
    <!-- CSS for styling -->
    <style>
        body, html {
            margin: 0;
            padding: 0;
            height: 100%;
            background-color: #1c1233; /* Set background color to black */
            display: flex;
        ...
        .button-container {
            ...
            bottom: 5%; /* 10% from the bottom */
            width: 70%; /* Same width as the color picker */
            
            height: 5%; /* Same width as the color picker */
            display: flex;
        ...
        button:hover {
            background-color: #008B8B; /* Dark Cyan background on hover/click */
        }
    </style>
</head>
<body>
    <!-- Container element for the color picker -->
    <div class="container">
        <div id="picker"></div>
    </div>
    ...
    <div class="button-container">
        <button id="gayButton" onclick="window.location.href = '/gay'">GAY</button>
        <button id="backButton" onclick="window.location.href = '/'">BACK</button>
    </div>

    <script>        
        // Create a new iro.js color picker instance
        var colorPickerWidth = Math.min(window.innerWidth, window.innerHeight) * 0.7;
        ...
        function sendRequest(input) {
            ...
            xhr.open('GET', '/action?act=' + input + '&r=' + r + '&g=' + g + '&b=' + b, true);
            xhr.send();
            console.log("action: "+input+" r: "+r+" g: "+g+" b: "+b);
        }
    </script>
</body>
</html>
...
.list ul a {
  display: block; /* Ensure the anchor tag occupies the entire width */
  color: inherit; /* Make sure the color is consistent with the list item */
  text-decoration: none; /* Remove any underlines from the anchor tag */
}

.list ul a li {
  display: block; /* Ensures the list item fills the entire anchor area */
}
</style>
</head>
<body>
...
<html>
    ...
    <style>
        * {
        ...
        .list ul a {
          display: block;
          /* Ensure the anchor tag occupies the entire width */
          color: inherit;
          /* Make sure the color is consistent with the list item */
          text-decoration: none;
          /* Remove any underlines from the anchor tag */
        }
  
        .list ul a li {
          display: block;
          /* Ensures the list item fills the entire anchor area */
        }
      </style>
    ...
    <div class="container">
        <body>
            <div class="list">
              ...
              <ul>
                <li onclick="triggerFileInput()">
                  <span>File chosen: </span>
                  <span id="filename">none</span>
                </li>        
                <p>
                  <span id="status">Firmware upload status</span>
                </p>
                <p>
                    <span id="loaded_n_total">Uploaded 0 bytes of 0</span>
                </p>
                <li onclick="triggerUpload()">
                  <span>UPLOAD</span>
                </li>
                <a href="/">
                  <li>
        ...
        <form enctype="multipart/form-data" class="form-container" id="upload_form" method="post">
            <div id="switch-container" hidden="true"><button class="button-o left selected" id="firmware-button" type="button">Firmware</button><button class="button-o right" id="filesystem-button" type="button">FileSystem</button></div><input onchange="handleFileInput(event)" hidden="true" accept=".bin,.bin.gz" class="input-file" id="file1" name="firmware" type="file"><br>
            <div class="progress" hidden="true"><span class="progress-bar" id="progressBar" style="width:0%"></span></div>
            <div class="additional-info">
            </div>
            <button id="button-send"hidden="true" class="button-send" type="submit" disabled="disabled">Upload</button>
            <hr hidden="true">
        </form>
        <script>
            ...
            function progressHandler(e) {
                ...
                _("progressBar").style = "width: " + e + "%", _("status").innerHTML = e + "% uploaded... please wait"
            }

            function completeHandler(e) {
                enableAll(), 0 <= e.target.responseText.indexOf("error") ? _("status").innerHTML = e.target.responseText : _("status").innerHTML = "Upload Success!", _("progressBar").value = 0
            }
            ...
            onlyFirmware && !0 === stringToBoolean(onlyFirmware) && (_("switch-container").style.display = "none"), _("upload_form").addEventListener("submit", e => {
                e.preventDefault(), uploadFile()
            }), _("firmware-button").addEventListener("click", function(e) {
                e.target.classList.add("selected"), _("filesystem-button").classList.remove("selected"), _("file1").name = "firmware"
            }), _("filesystem-button").addEventListener("click", function(e) {
                e.target.classList.add("selected"), _("firmware-button").classList.remove("selected"), _("file1").name = "filesystem"
            }), _("file1").addEventListener("change", function(e) {
                var t = _("file1").files[0];
                t && t.name ? _("button-send").disabled = !1 : _("button-send").disabled = !0, _("status").innerHTML = "Firmware Upload!", _("loaded_n_total").innerHTML = "Uploaded 0 bytes of 0"
            })
        </script>
    </div>
</html>>
...
<head>
  <style>
    ...
    .slider-container input[type="range"]::-webkit-slider-thumb {
        ...
        height: 32px; /* Increase the height of the thumb */
        background: #008B8B;
        cursor: pointer;
        border-radius: 50%; /* Keep the thumb circular */
    }
...
<body>
    <div class="container">
        ...
        <div class="slider-container">
            ...
            <input id="repeat" type="range" min="100" max="200" value="100">
        </div>
        <div class="slider-container">
            <label for="uniformity">UNIFORMITY</label>
            <input id="uniformity" type="range" min="0" max="100" value="0">
        </div>
        <div class="slider-container">
            <label for="brightness">BRIGHTNESS</label>
            <input id="brightness" type="range" min="0" max="255" value="255">
        </div>
        <div class="button-container">
            <button id="backButton" onclick="window.location.href = '/buttons'">SOLID COLOR</button>
        </div>
    </div>
    <script>
        var sliders = {
        ...
        function sendRequest() {
            var xhr = new XMLHttpRequest();
            xhr.open('GET', `/actionGAY?s=${sliders.speed}&d=${sliders.density}&r=${sliders.repeat}&u=${sliders.uniformity}&b=${sliders.brightness}`, true);
            xhr.send();
            console.log(`Speed: ${sliders.speed}, Density: ${sliders.density}, Repeat: ${sliders.repeat}, Uniformity: ${sliders.uniformity}, Brightness: ${sliders.brightness}`);
        }
    </script>
</body>
</html>
